...
/**
 * Syncs the AsyncStorage session cookie into the WebView jar before page JS runs.
 * We intentionally do not verify via CookieManager.get(): on iOS, WKHTTPCookieStore
 * getAllCookies + the library's domain filter can omit cookies that the WebView will
 * still send, so "sync never succeeds" was possible while retries only burned time.
 * A short delay lets setCookie completion handlers settle before the first navigation.
 *
