      ...
      try {
        ...
        if (!user || !user.get('email')) {
          ...
          if (!group) {
            ...
            const formattedPurchaseDate = purchaseDate.toLocaleDateString(mapLocaleToSendWithUS(userLocale), {
              year: 'numeric',
              month: 'long',
              day: 'numeric'
            })
            ...
            if (isSubscription && stripeSubscriptionId) {
              try {
                ...
                if (subscription.current_period_end) {
                  ...
                  renewalDate = renewalDateObj.toLocaleDateString(mapLocaleToSendWithUS(userLocale), {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                  })
            ...
            if (isTrackPurchase && track) {
              ...
              if (isSubscription) {
                ...
                if (duration && duration !== 'lifetime') {
                  ...
                  emailData.expires_at = expiresAtDate.toLocaleDateString(mapLocaleToSendWithUS(userLocale), {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                  })
            ...
            if (admins && admins.models && admins.models.length > 0) {
              ...
              await Promise.all(admins.models.map(async (adminMembership) => {
                ...
                const cancelledAtFormattedForAdmin = cancelledAt.toLocaleDateString(
                  mapLocaleToSendWithUS(adminLocale),
                  {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
