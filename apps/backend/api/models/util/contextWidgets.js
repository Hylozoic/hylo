...
export function reorderTree({priorWidgetState = {}, newWidgetPosition, allWidgets}) {
  ...
  if (!replacedWidget) {
    ...
    newPeers.push({ id: newWidgetPosition.id, order: newOrder + 1, parentId: newWidgetPosition.parentId || null })
    ...
    newPeers = newPeers.reduce((acc, widget) => {
      if (widget.order > replacedWidget.order) {
        ...
        acc.push({ ...widget, order: widget.order + 1 })
        acc.push({ id: newWidgetPosition.id, order: widget.order, parentId: widget.parentId || null })
      } else {
        acc.push(widget)
      }
      return acc
