exports.up = async function (knex) {
  console.log('Switching from home topic to general topic and chat')
  ...
  groupsWithoutGeneralTag.rows.forEach(async (row) => {
    ...
    const query = `INSERT INTO groups_tags (group_id, tag_id, is_default, visibility, num_followers, created_at, updated_at)
  ...
  peopleNotFollowingGeneralTag.rows.forEach(async (row) => {
    ...
    const query = `INSERT INTO tag_follows (group_id, user_id, tag_id, new_post_count, created_at, updated_at)
                   VALUES (${groupId}, ${userId}, ${generalId}, 0, current_timestamp, current_timestamp) ON CONFLICT DO NOTHING;`
    await knex.raw(query)
  })

  // Home chat becomes general chat
  const query = `UPDATE context_widgets
                 SET view_chat_id = ?
                 WHERE view_chat_id = ?
                 AND type = 'chat';`
  await knex.raw(query, [generalId, homeId])
}
...
exports.down = async function (knex) {
  ...
  // General chat becomes home chat
  const query = `UPDATE context_widgets
                 SET view_chat_id = ?
                 WHERE view_chat_id = ?
                 AND type = 'chat';`
  await knex.raw(query, [homeId, generalId])
}
