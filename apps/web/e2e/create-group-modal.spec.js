...
test.describe('Create Group modal', () => {
  test('opens from the global nav and generates a handle from the name', async ({ page }) => {
    ...
    await page.screenshot({ path: 'e2e-screenshots/create-group-01-modal-empty.png' })
    ...
    await page.screenshot({ path: 'e2e-screenshots/create-group-02-handle-generated.png' })
  })
  ...
  test('reveals advanced settings inline', async ({ page }) => {
    ...
    await page.screenshot({ path: 'e2e-screenshots/create-group-03-advanced-open.png' })
  })
  ...
  test('flags a handle that is already taken', async ({ page }) => {
    ...
    await expect(dialog.getByText('This URL already exists. Try another.')).toBeVisible()
    await expect(dialog.getByRole('button', { name: /Create Group/i })).toBeDisabled()
    await page.screenshot({ path: 'e2e-screenshots/create-group-04-handle-taken.png' })
  })
