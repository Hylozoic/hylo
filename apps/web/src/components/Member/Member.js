...
import { fetchGroupJoinQuestionAnswers } from 'routes/Members/Members.store'
...
function Member ({
  ...
  const removeOnClick = (e, id, name, removeMember) => {
    e.preventDefault()
  ...
  useEffect(() => {
    if (canSeeJoinAnswers && showAnswers && group?.id && member?.id) {
      const fetchKey = `${group.id}-${member.id}`

      if (!fetchedRef.current.has(fetchKey) && !isLoadingAnswers) {
        setIsLoadingAnswers(true)
        fetchedRef.current.add(fetchKey)

        dispatch(fetchGroupJoinQuestionAnswers({ groupId: group.id, userId: member.id }))
          .then((response) => {
            const answers = response?.payload?.data?.groupJoinQuestionAnswers?.items || []
            setJoinAnswers(answers)
  ...
  }, [canSeeJoinAnswers, showAnswers, group?.id, member?.id, dispatch])
    ...
    <div className={cn('flex flex-col gap-2 bg-card/100 rounded-lg p-2 shadow-lg hover:bg-card/100 transition-all hover:scale-102 relative overflow-hidden', className)} data-testid='member-card'>
      {(currentUserResponsibilities.includes(RESP_REMOVE_MEMBERS)) &&
        <Dropdown
          ...
          items={[{ icon: 'Trash', label: t('Remove'), onClick: (e) => removeOnClick(e, id, name, removeMember) }]}
        />}
      ...
      {canSeeJoinAnswers && showAnswers && joinAnswers?.length > 0 && (
      ...
      {canSeeJoinAnswers && showAnswers && isLoadingAnswers && (
        <div className='flex flex-col gap-2 z-10 relative'>
          <div className='text-sm font-semibold text-foreground/80 border-t border-foreground/20 pt-2'>
            {t('Loading join question responses...')}
          </div>
        </div>
      )}
