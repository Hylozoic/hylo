...
export default function EventDate ({ startTime }) {
  ...
  return (
    <div className='w-16 h-16 flex items-center flex-col justify-center bg-white rounded-lg'>
      ...
      <div className='bg-white text-darkening border-b-2 border-l-2 border-r-2 border-error/20 rounded-b-md h-1/2 w-full uppercase items-center justify-center flex font-bold text-2xl'>{start.toFormat('d')}</div>
    </div>
  )
}
