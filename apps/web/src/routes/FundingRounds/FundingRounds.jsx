import { BadgeDollarSign, UserPlus, UserMinus } from 'lucide-react'
...
/** Resolve a space URL for a funding round (round.group is the FR space). */
function roundSpaceUrl (round, groupSlug) {
  return fundingRoundUrl(round.id, { groupSlug, space: round.group, tab: 'funding-round-submissions' })
}
...
function FundingRounds () {
  ...
  const handleJoinRound = useCallback(async (e, round) => {
    e.preventDefault()
    e.stopPropagation()
    await dispatch(joinFundingRound(round.id))
    navigate(roundSpaceUrl(round, groupSlug))
  }, [dispatch, groupSlug, navigate])
  ...
  return (
    <div className='p-4 max-w-[750px] mx-auto flex flex-col gap-2'>
      ...
      {roundsToDisplay.map(fr => {
        ...
        const spaceHref = roundSpaceUrl(fr, groupSlug)
        const description = fr.group?.description || fr.description
        const participants = (() => {
        ...
        return (
          <div key={fr.id} className='rounded-xl text-foreground p-4 flex flex-col sm:flex-row sm:items-start gap-3 transition-all bg-card/50 hover:bg-card/100 border-2 border-card/30 shadow-xl hover:shadow-lg relative hover:z-[2] hover:scale-101 duration-400'>
            <Link to={spaceHref} className='flex-1 min-w-0 text-foreground hover:text-foreground'>
              <div className='flex flex-col items-start gap-1 pb-1'>
                <div className='flex flex-wrap items-center gap-x-3 gap-y-1 text-xl font-semibold'>
                  <span>{fr.title}</span>
                  <span className={cn('px-2 py-0.5 rounded-full text-xs font-medium whitespace-nowrap', phase.badgeClass)}>
                    {phase.label}
                  </span>
                </div>
                {description && <div className='text-sm text-foreground/80 line-clamp-2'>{description.replace(/<[^>]*>/g, '')}</div>}
              </div>
            </Link>

            <div className='flex flex-col items-start sm:items-end gap-3 shrink-0'>
              <Link to={spaceHref} className='flex items-center gap-3 px-3 py-1 rounded-full bg-foreground/5 text-xs font-medium text-foreground'>
              ...
              {fr.publishedAt && (
                fr.isParticipating
                  ...
                  : (
                    <button
                      onClick={(e) => handleJoinRound(e, fr)}
