...
/**
 * TrackPaywallOfferingsSection Component
 *
 * Displays available offerings for an access-controlled track and allows users
 * to purchase access via Stripe checkout.
 * Filters offerings to only show those that grant access to this specific track.
 */
export default function TrackPaywallOfferingsSection ({ track }) {
  ...
  // Get the group from route params (tracks belong to a single group)
  const group = useSelector(state => getGroupForSlug(state, routeParams.groupSlug))

  // Filter offerings to only those that grant access to this track
  const trackAccessOfferings = useMemo(() => {
    if (!offerings || !track?.id) return []
  ...
  useEffect(() => {
    if (!track?.accessControlled || !group?.id) {
      setLoading(false)
      return
    }
    ...
    loadOfferings()
  }, [dispatch, group?.id, track?.accessControlled])
  ...
  if (trackAccessOfferings.length === 0) {
    return (
      <div className='border-2 border-dashed border-foreground/20 rounded-xl p-4 text-center'>
  ...
  return (
    <div className='border-2 border-dashed border-foreground/20 rounded-xl p-4'>
      <div className='flex items-center gap-2 mb-4'>
        ...
        <h3 className='text-lg font-semibold'>{t('This track requires a fee to access')}</h3>
      </div>
