...
export default function MenuLink ({ badgeCount = null, to, children, onClick, externalLink, className, isEditing }) {
  ...
  return (
    <Link to={to} onClick={handleClick} className={cn('text-foreground text-sm focus:text-foreground relative', className, { 'opacity-100 border-selected bg-card/100 font-bold': isCurrentLocation }, { 'border-accent': badgeCount > 0 })}>
      {children}
