...
import { debounce, get, includes, isEmpty, trim } from 'lodash/fp'
...
export default function ChatRoom (props) {
  ...
  const postsPastSelector = useSelector(state => getPosts(state, fetchPostsPastParams))
  const postsPast = useMemo(() => (postIdToStartAt || groupTopic?.lastReadPostId) ? postsPastSelector : [], [groupTopic?.lastReadPostId, postIdToStartAt, postsPastSelector])
  ...
  }, [fetchPostsPastParams, pending, hasMorePostsPast])
  ...
  }, [fetchPostsFutureParams, pending, hasMorePostsFuture])
  ...
  }, [navigate, routeParams, location.state, querystringParams])

  const toggleGroupTopicSubscribeAction = useCallback((groupTopic) => dispatch(toggleGroupTopicSubscribe(groupTopic)), [dispatch])

  const updateGroupTopicLastReadPostAction = useCallback((groupTopicId, postId) => dispatch(updateGroupTopicLastReadPost(groupTopicId, postId)), [])
  ...
  useResizeDetector({ handleWidth: false, targetRef: chatsRef, onResize: handleResizeChats })
  ...
  const postChatMessage = useEventCallback(async () => {
    ...
    await createPostAction(postToSave)
    setNewPost(emptyPost)
                        ...
                        : (
                          <div className={cx(styles.cardItem, { [styles.expanded]: expanded })}>
                            <PostCard
                              ...
                              respondToEvent={respondToEventAction}
