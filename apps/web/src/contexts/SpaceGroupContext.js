...
export function useGroupRouteOpts () {
  ...
  // resolve the space's full slug from the parent group's space views
  ...
  const resolvedSpaceGroupSlug = useMemo(() => {
    ...
    if (!needsLookup || !groupViews) return null
    const spaceView = groupViews.find(v =>
      v.type === 'space' &&
      localSpaceSlug(parentGroupSlug, v.linkedGroup?.slug) === spaceSlug
    )
    return spaceView?.linkedGroup?.slug || null
  }, [spaceGroupSlug, needsLookup, groupViews, parentGroupSlug, spaceSlug])
