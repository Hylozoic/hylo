...
describe('sanitizeHTML', () => {
  it('keeps YouTube and Vimeo embed videos and drops scripts', () => {
    const html = TextHelpers.sanitizeHTML(
      '<p>hi</p><video data-type="embed" src="https://player.vimeo.com/video/70509133"></video><script>alert(1)</script>'
    )
    expect(html).toContain('<p>hi</p>')
    expect(html).toContain('player.vimeo.com/video/70509133')
    expect(html).not.toContain('script')
  })
})

describe('richTextToHTML', () => {
  it('converts legacy markdown and leaves editor HTML in place', () => {
    expect(TextHelpers.richTextToHTML('**bold**')).toBe('<p><strong>bold</strong></p>\n')
    expect(TextHelpers.richTextToHTML('<p>Hello <strong>world</strong></p>')).toBe('<p>Hello <strong>world</strong></p>')
  })
})
